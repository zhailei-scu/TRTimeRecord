...
void PatientInfoSetting::resetAction(){
    ...
    const std::map<unsigned int,QString> *patientInfo = ConfigLoader::getInstance()->getThePatientInfoPatten();

    if(patientInfo){
        for(std::map<unsigned int,QString>::const_iterator it = patientInfo->begin();
                                                           it != patientInfo->end();
                                                           it++){
            this->uiForm->tableWidget->insertRow(it->first);
            this->uiForm->tableWidget->setItem(it->first,0,new QTableWidgetItem(it->second));
            this->uiForm->tableWidget->setItem(it->first,1,new QTableWidgetItem(it->second));
        }
    }
}
